...
function RegisterPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await navigate({ to: "/dashboard/buat" });
    } catch (err) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-background px-6">
      <form onSubmit={submit} className="w-full max-w-md rounded-2xl bg-card p-7 hairline shadow-elegant">
        ...
        <h1 className="mt-8 font-serif text-4xl">Daftar</h1>
        <p className="mt-2 text-sm text-muted-foreground">Buat akun untuk menyimpan draft undangan ke database.</p>
        <div className="mt-6 space-y-4">
          <label className="block">
            <span className="mb-1.5 block text-xs uppercase tracking-widest text-muted-foreground">Nama</span>
            <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="field" required />
          </label>
          <label className="block">
          ...
          <label className="block">
            ...
            <input type="password" minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className="field" required />
          </label>
        </div>
